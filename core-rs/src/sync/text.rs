...
fn render_template_root_readme_with_newlines(config: &ToolConfig, newline: &str) -> String {
    ...
    [
        ...
        "- a scaffold you can create, upgrade, adopt, and keep in sync".to_string(),
        ...
        String::new(),
        "Prepare the embedded-template artifact before direct Rust build or test commands:".to_string(),
        String::new(),
        "- `node scripts/stage-distribution-assets.mjs embedded-template`".to_string(),
        String::new(),
        "- `cargo run --manifest-path core-rs/Cargo.toml -- create <target>`".to_string(),
        "- `cargo run --manifest-path core-rs/Cargo.toml -- validate --json`".to_string(),
        "- `cargo run --manifest-path core-rs/Cargo.toml -- inspect --json`".to_string(),
        "- `cargo run --manifest-path core-rs/Cargo.toml -- upgrade --plan --json`".to_string(),
        "- `cargo run --manifest-path core-rs/Cargo.toml -- sync --check --json`".to_string(),
...
fn render_releases_doc_with_newlines(config: &ToolConfig, newline: &str) -> String {
    ...
    if is_template_project(config) {
        return [
            ...
            String::new(),
            "The checked-in runtime package folders still use target-oriented paths such as `wrapper-js/platform-packages/ossplate-win32-x64/`, but the published npm package name is derived from the public package identity plus the runtime suffix. For Windows that means:".to_string(),
            String::new(),
            "- internal target and folder: `win32-x64`".to_string(),
            "- published runtime package: `ossplate-windows-x64`".to_string(),
            String::new(),
            "## Release Flow".to_string(),
            String::new(),
            "Use the repo-local planner before any publish attempt:".to_string(),
            String::new(),
            "```bash".to_string(),
            "cargo run --manifest-path core-rs/Cargo.toml -- publish --plan --json".to_string(),
            "```".to_string(),
            String::new(),
            "That plan reports:".to_string(),
            String::new(),
            "- the helper invocation `publish` will run".to_string(),
            "- which registries are selected".to_string(),
            "- whether the run is `--dry-run`".to_string(),
            "- whether `--skip-existing` recovery mode is enabled".to_string(),
            String::new(),
            "The executable publish surface is:".to_string(),
            String::new(),
            "```bash".to_string(),
            "cargo run --manifest-path core-rs/Cargo.toml -- publish [--dry-run] [--skip-existing] [--registry <cargo|npm|python|all>]".to_string(),
            "```".to_string(),
            String::new(),
            "Use cases:".to_string(),
            String::new(),
            "- `--dry-run` for local operator rehearsal without registry mutation".to_string(),
            "- `--registry <name>` to limit publication to one ecosystem".to_string(),
            "- `--skip-existing` for rerun-safe recovery when some artifacts already exist upstream".to_string(),
            String::new(),
            "`publish` is intentionally source-checkout only. It orchestrates release helpers from the checked-out repo and does not mutate git state for you.".to_string(),
            String::new(),
            "If you run direct Rust build or test commands in the canonical repo outside `verify.sh`, prepare the embedded-template artifact first:".to_string(),
            String::new(),
            "```bash".to_string(),
            "node scripts/stage-distribution-assets.mjs embedded-template".to_string(),
            "```".to_string(),
            String::new(),
            "## Verification Before Publish".to_string(),
            String::new(),
            ...
            "```".to_string(),
            String::new(),
            "The current verification path also checks:".to_string(),
            String::new(),
            "- generated-project dogfooding from the scaffold payload".to_string(),
            "- resolved versus placeholder JS lockfile state".to_string(),
            "- package dry-runs for the top-level JS package and runtime packages".to_string(),
            "- Python wheel build and wrapper tests".to_string(),
            String::new(),
            "If a publish attempt needs recovery, rerun the planner first and then rerun `publish` with the narrowest registry scope and `--skip-existing` only when the upstream state already proves that artifact exists.".to_string(),
            String::new(),
        ]
        .join(newline);
    }
